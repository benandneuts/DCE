...
//    	CarteInfluence carte = plateau.getColonne(indexColonne).getCartesInfluences()[plateau.getColonne(indexColonne).nombreCartesInfluences() - 2];
    ...
    public void remplirReserve(Joueur joueur) {
    	if(joueur.reserveNulle()) {
    		for(int i = 0; i < joueur.getReserve().length; i++) {
    			if(joueur.getDefausse()[i] != null) {
    				joueur.getReserve()[i] = joueur.getDefausse()[i];
    			}
        	}
    	}
    	joueur.setDefausse(new CarteInfluence[25]);
    }
		...
		for(int i = 0; i<plateau.getColonnes().length; i++) {
			for(int j = 0 ; j < plateau.getColonnes()[i].getCartesInfluences().length ; j++) {
				if(plateau.getColonnes()[i].getCartesInfluences()[j]!=null) {
			    	...
			    	//System.out.println(plateau.getColonnes()[i].getCartesInfluences()[j].getEstVisible());
				}
			}
		}
    ...
    public void activerCartesARetardement() throws Exception {
    	...
    	for (Colonne colonne : this.getPlateau().getColonnes()) {
    		int index = colonne.getIndexDerniereCarte();
    		//System.out.println("Derniere carte de la colonne : " + index);
    		for(int i = 0; i<index; i++) {
    			CarteInfluence carte = colonne.getCarteInfluence(i);
    		...
    		for(int i = 0; i<index; i++) {
    			...
    			if(carte instanceof Magicien && !((Magicien) carte).estDesactivee()) {
    				...
    				((CarteARetardement) carte).setDesactivee(true);
    				index = colonne.getIndexDerniereCarte();
    			}
    		...
    		for(int i = 0; i<index; i++) {
    			CarteInfluence carte = colonne.getCarteInfluence(i);
    			if(carte instanceof CarteARetardement && !((CarteARetardement) carte).estDesactivee() && this.getPlateau().getColonne(this.getPlateau().getIndexColonneCarte(carte)).getIndexDerniereCarte() != -1) {
    				//System.out.println("Colonne : " + this.getPlateau().getIndexColonneCarte(carte) + ", Carte : " + carte.getNom() + ", Valeur : " + carte.getValeur() + ", Joueur : " + this.getIndexJoueurParCouleur(carte.getCouleur()));
	...
	public void eliminerCarteData(int indexColonne, CarteInfluence carte) throws Exception {
    	...
    	//System.out.println(carte.getEstVisible());
		this.getPlateau().setColonnes(newColonnes);
	}
	
	public void eliminerCarteDataSansDefausse(int indexColonne, CarteInfluence carte) throws Exception {
		Colonne[] newColonnes = this.getPlateau().getColonnes();
		newColonnes[indexColonne] = this.getPlateau().eliminerCartePlateau(indexColonne, carte);
		this.getPlateau().setColonnes(newColonnes);
	}
...
//			System.out.println("JOUEUR : " + joueur.getPseudo() + "------     SCORE : " + joueur.getScore());
	...
	/**
	 * Retourne l'index de la prochaine colonne pour l'Explorateur.
	 * 
	 * @param indexColonneActuelle L'index de la colonne ou se trouve l'Explorateur.
	 * 
	 * @return l'index de la prochaine colonne.
	 * 
	 * @since 1.0
	 */
	public int prochaineColonne(int indexColonneActuelle) {
		int nbColonne = this.getPlateau().getColonnes().length;
		int indexColonneVisee = (indexColonneActuelle+1)%nbColonne; 
			indexColonneVisee = detecteurColonneComplete(indexColonneVisee, nbColonne);
		
		return indexColonneVisee;
	}
	
	/**
	 * Detecte si les colonnes suivantes sont completes.
	 * 
	 * @param colonneSus La colonne qu'on verifie si elle est complete.
	 * 
	 * @param nbColonne Le nombre de colonne sur le plateau.
	 * 
	 * @return le nombre de colonne successive complete.
	 * 
	 * @since 1.0
	 */
	public int detecteurColonneComplete(int colonneSus, int nbColonne){

		for(int i = 0; i < nbColonne; i++) {
			if (this.getPlateau().getColonne(colonneSus).estPleine()) {
				colonneSus = (colonneSus + i) % nbColonne;
			}
			else
				return colonneSus;
		}
