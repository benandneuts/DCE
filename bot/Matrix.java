...
public class Matrix{
   ...
   *@return
   */
  public Matrix ajouter(float value){
  ...
  public void set_value_line(float[] array){
    ...
    if(x != 1){
      erreur("la matrice est plus grande que 1");
    }else{
   ...
   * @param erreur_message le message d'erreur
   */
  public static void erreur(String erreur_message){
    System.out.println("");
    System.out.print("=======erreur :");
    System.out.println(erreur_message);
    System.out.println("");
}
   ...
   *
   */
  public static Matrix multiplier(Matrix factor1, Matrix factor2){
  if(factor1.y != factor2.x){
   erreur(" factor1 y ("+factor1.y+") et factor2 x ("+factor2.x+") ne correspondent pas");
   erreur(factor1.x+"   "+factor1.y+" | " + factor2.x+"   "+factor2.y);
...
public static Matrix plus(Matrix term1, Matrix term2){
  if(term1.x != term2.x){
    erreur(" term1 x ("+term1.x+") et term2 x ("+term2.x+") ne correspondent pas");
    erreur(term1.x+"   "+term1.y+" | " + term2.x+"   "+term2.y);
    return(new Matrix(1,1));
  }else{
    if(term1.y != term2.y){
    erreur(" term1 y et term2 y ne correspondent pas");
    erreur(term1.x+"   "+term1.y+" | " + term2.x+"   "+term2.y);
    return(new Matrix(1,1));
    }else{
 ...
 * @return
 */
public static Matrix soustraire(Matrix term1, Matrix term2){
  if(term1.x != term2.x){
    erreur(" term1 x et term2 x ne correspondent pas");
    return(new Matrix(1,1));
  }else{
    if(term1.y != term2.y){
    erreur(" term1 y et term2 y ne correspondent pas");
 ...
 */
	public static Matrix fusion(Matrix factor, Matrix factor_2){
	  ...
	  if(factor.x != factor_2.x){
	   erreur(" term1 x ("+factor.x+") et term2 x ("+factor_2.x+") ne correspondent pas");
	    erreur("  "+ factor.x+" |  " +    factor.y);
	    erreur("  "+ factor_2.x+" |  " +    factor_2.y);
	    return(new Matrix(1,1));
	  }else{
