		...
		class TaskDelay extends TimerTask { // Timer pour voir la fin de la manche
			public void run() {
				...
				Platform.runLater(() -> {
					...
					try {
						int indexColonne;
						indexColonne = data.getPlateau().getIndexColonneCarte(self);
						int indexAssassin = data.getPlateau().getColonne(indexColonne).getIndexCarteInfluence(self);
...
//											carteDetruite = data.getPlateau().getColonne(indexColonne).getCarteInfluence(1);
							...
							//System.out.println("Carte assassin activée, Ref : " + self + ", Joueur : " + data.getIndexJoueurParCouleur(self.getCouleur()) + ", Index : " + indexAssassin + ", Colonne : " + data.getPlateau().getIndexColonneCarte(self)); 
