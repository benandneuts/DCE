			...
			for(int i = 0 ; i<cartes.length ; i++) {
				if (cartes[i] instanceof CarteDouble) {
					...
					if(cartes[i] != null && cartes[i].getValeur() > 0 && cartes[i] != null && cartes[i].getValeur() < 9.5 && cartes[i] != this && !(cartes[i] instanceof Sosie)) { 
						...
						if (cartes[i] != null) {
							cartes[i].setEstVisible(true);
							//System.out.println("V Non Suppression de la Carte : " + cartes[i].getNom() + ", Valeur : " + cartes[i].getValeur() + ", Joueur: " + data.getIndexJoueurParCouleur(cartes[i].getCouleur()) + ", Visibilité : " + cartes[i].getEstVisible());
						}
					}
				} 
			}
